...
class BicepCurl:
    ...
    def check_form(self, lmList):
        elbow_angle_threshold_up = 160
        elbow_angle_threshold_down = 30

        elbow_angle = self.calculate_angle(lmList[11], lmList[13], lmList[15])

        elbow_percentage_up = min(max((elbow_angle - 30) / (elbow_angle_threshold_up - 30) * 100, 0), 100)
        elbow_percentage_down = min(max((elbow_angle - 30) / (elbow_angle_threshold_down - 30) * 100, 0), 100)

        if self.dir == 1:  # Upward movement
            self.percentage = elbow_percentage_up
        else:  # Downward movement
            self.percentage = elbow_percentage_down

        self.form_good = (elbow_percentage_up >= 75) or (elbow_percentage_down >= 75)
        ...
        while True:
            try:
                ...
                if len(lmList) != 0:
                    ...
                    if lmList[15][2] < lmList[13][2]:  # Hand above elbow
                        if self.dir == 0:
                            self.count += 1
                            self.dir = 1
                    elif lmList[15][2] > lmList[13][2]:  # Hand below elbow
                        self.dir = 0
...
if __name__ == "__main__":
    ...
    bc = BicepCurl(video_path, user_name)
    bc.run()
