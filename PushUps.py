...
class Pushup:
    ...
    def check_form(self, lmList):
        shoulder_angle_threshold = 80
        elbow_angle_down_threshold = 60
        elbow_angle_up_threshold = 160

        shoulder_angle = self.calculate_angle(lmList[11], lmList[13], lmList[15])
        elbow_angle = self.calculate_angle(lmList[13], lmList[15], lmList[17])

        shoulder_percentage = min(max((shoulder_angle - 90) / (shoulder_angle_threshold - 90) * 100, 0), 100)
        elbow_percentage_down = min(max((elbow_angle - 50) / (elbow_angle_down_threshold - 50) * 100, 0), 100)
        elbow_percentage_up = min(max((elbow_angle - 140) / (elbow_angle_up_threshold - 140) * 100, 0), 100)

        if self.dir == 1:  # DOWN
            self.percentage = elbow_percentage_down
        else:  # UP
            self.percentage = elbow_percentage_up

        self.form_good = (shoulder_percentage >= 75) and ((elbow_percentage_down >= 75) or (elbow_percentage_up >= 75))
        ...
        while True:
            try:
                ...
                if len(lmList) != 0:
                    ...
                    if lmList[13][2] > lmList[11][2]:  # Elbow below shoulder
                        if self.dir == 0:
                            self.count += 1
                            self.dir = 1
                    elif lmList[13][2] < lmList[11][2]:  # Elbow above shoulder
                        self.dir = 0
...
if __name__ == "__main__":
    ...
    pu = Pushup(video_path, user_name)
    pu.run()
